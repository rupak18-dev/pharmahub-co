...
export function DashboardWidgets({ db }: DashboardWidgetsProps) {
  ...
  const lowStock = db.medicines
    .filter(m => {
      const stock = db.batches.filter(b => b.medicineId === m.id).reduce((sum, b) => sum + b.currentStock, 0);
  ...
  const uniqueCustomers = Array.from(new Set(db.sales.filter(s => s.customerName).map(s => s.customerName)));
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
      ...
      <div className="rounded-2xl border border-border bg-card shadow-sm flex flex-col">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          ...
          <Link to="/dashboard/sales" className="text-xs font-medium text-blue-600 hover:underline">View all</Link>
        </div>
              ...
              {recentSales.map((sale) => (
                <li key={sale.id} className="flex items-center justify-between px-4 py-3 hover:bg-muted/30 transition-colors">
                  ...
                  <div className="text-right">
                    ...
                    <Badge variant={sale.status === "completed" ? "success" : "secondary"} className="text-[10px] mt-1 h-4 px-1.5">
                      {sale.status}
                    </Badge>
                  </div>
                </li>
      ...
      <div className="rounded-2xl border border-border bg-card shadow-sm flex flex-col">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          ...
          <Link to="/dashboard/purchases" className="text-xs font-medium text-violet-600 hover:underline">View all</Link>
        </div>
        <div className="p-0 flex-1">
          {pendingPOs.length > 0 ? (
            <ul className="divide-y divide-border">
      ...
      <div className="rounded-2xl border border-border bg-card shadow-sm flex flex-col">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          ...
          <Link to="/dashboard/inventory" className="text-xs font-medium text-amber-600 hover:underline">View all</Link>
        </div>
        <div className="p-0 flex-1">
          {lowStock.length > 0 ? (
            <ul className="divide-y divide-border">
              {lowStock.map((med) => {
                const stock = db.batches.filter(b => b.medicineId === med.id).reduce((sum, b) => sum + b.currentStock, 0);
                return (
                  <li key={med.id} className="flex items-center justify-between px-4 py-3 hover:bg-muted/30 transition-colors">
                    <div>
                      <p className="text-sm font-medium text-foreground">{med.name}</p>
                      <p className="text-xs text-muted-foreground">Threshold: {med.reorderThreshold}</p>
                    </div>
                    <div className="text-right">
                      <p className={`text-sm font-bold ${stock === 0 ? 'text-destructive' : 'text-amber-500'}`}>
                        {stock} units
                      </p>
                    </div>
                  </li>
            ...
            <div className="p-4 text-center text-sm text-muted-foreground">Stock levels are healthy.</div>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card shadow-sm flex flex-col">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          ...
          <Link to="/dashboard/expiry" className="text-xs font-medium text-rose-600 hover:underline">View all</Link>
        </div>
        <div className="p-0 flex-1">
          {expiring.length > 0 ? (
            <ul className="divide-y divide-border">
              {expiring.map((batch) => {
                ...
                return (
                  <li key={batch.id} className="flex items-center justify-between px-4 py-3 hover:bg-muted/30 transition-colors">
                    <div>
                    ...
                    <div className="text-right">
                      <p className={`text-sm font-bold ${isExpired ? 'text-destructive' : 'text-amber-500'}`}>
            ...
            <div className="p-4 text-center text-sm text-muted-foreground">No near expiry batches.</div>
          )}
        </div>
      </div>
          ...
          {recentCustomers.length > 0 ? (
            <ul className="divide-y divide-border">
              {recentCustomers.map((customer, idx) => (
                <li key={idx} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/30 transition-colors">
            ...
            <div className="p-4 text-center text-sm text-muted-foreground">No recent customers.</div>
          )}
        </div>
      </div>
