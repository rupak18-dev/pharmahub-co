...
import type { ModuleKey, PermissionAction, Profile, RoleName } from "@/lib/types";
...
export function InviteUserWizard({ isOpen, onClose }: InviteUserWizardProps) {
  ...
  // Step 2 State
  const [selectedRole, setSelectedRole] = useState<RoleName>("Pharmacist");
  ...
  const toggleModuleSelection = (modKey: ModuleKey) => {
  ...
  const toggleActionPill = (modKey: ModuleKey, actionKey: PermissionAction) => {
    setCustomPermissions((prev) => {
      const next = structuredClone(prev);
            ...
            {[
              ...
              return (
                <div
                  ...
                  className={`flex flex-col gap-1 border-t-2 pt-2 text-[11px] font-medium transition-all ${
