...
import type { Audit, AuditCount } from "@/lib/types";
import type { ScopeItem } from "@/lib/audit";
import type { CountExtra } from "@/routes/_authenticated.dashboard.audit";
...
function resizeImage(dataUrl: string, maxSize = 320): Promise<string> {
...
export function LiveAudit({
  ...
  onCount: (batchId: string, qty: number, extra?: CountExtra) => void;
  onSkip: (batchId: string) => void;
  onFlag: (batchId: string, extra?: CountExtra) => void;
  onPause: () => void;
  onResume: () => void;
  onSubmit: () => void;
  onSwitchAudit: (id: string) => void;
}) {
