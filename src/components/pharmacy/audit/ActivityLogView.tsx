...
import { DEFAULT_ACTIVITY_FILTERS, filterActivityLogs, type ActivityFilters } from "@/lib/audit";
import type { ActivityLog, Audit, Profile } from "@/lib/types";

const ENTITY_META: Record<
  string,
  { label: string; icon: typeof ClipboardCheck; tone: string; bg: string }
> = {
  audit: {
    label: "Audit Session",
    icon: ClipboardCheck,
...
function formatStatusValue(val: unknown) {
  if (val == null) return "";
  const s = String(val);
...
export function ActivityLogView({
  ...
  const [f, setF] = useState<ActivityFilters>(DEFAULT_ACTIVITY_FILTERS);
  ...
  const parseLogSubject = (log: ActivityLog) => {
