...
import type { ExpiryNotification } from "@/lib/expiry";
...
export function NotificationsPopover({
  ...
  onMarkAllRead: () => void;
  onJump: (batchId: string) => void;
}) {
  const unread = notifications.filter((n) => !readIds.has(n.id)).length;
  const groups: { label: string; items: ExpiryNotification[] }[] = [];
      ...
      <PopoverContent align="end" className="w-[22rem] p-0">
        ...
        <div className="max-h-80 overflow-y-auto">
          {notifications.length === 0 ? (
            ...
            groups.map((g) => {
              const Icon = groupIcons[g.label as keyof typeof groupIcons] ?? Clock3;
              return (
