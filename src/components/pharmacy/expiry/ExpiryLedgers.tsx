...
import type { CreditNoteEntry, WriteOffEntry } from "@/lib/types";

const CN_STYLES: Record<CreditNoteEntry["status"], string> = {
...
export function ExpiryLedgers({
  ...
  currency: string;
  onMarkReceived: (id: string, creditNoteNo?: string) => void;
  ...
  const exportWriteOffs = (format: "csv" | "xls") => {
    if (format === "csv") {
  ...
  const exportCreditNotes = (format: "csv" | "xls") => {
    if (format === "csv") {
