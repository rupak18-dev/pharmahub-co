...
export const handle = { title: "Medicine Details · PharmaHub" };
export default function MedicineDetailsPage() {
  ...
  const medBatches = useMemo(() => {
    return med ? data.batches.filter((b) => b.medicineId === med.id) : [];
  }, [data.batches, med]);
  const genericAlternatives = useMemo(() => {
    ...
    if (alts.length === 0) {
      alts = [...data.medicines]
        .filter((m) => m.id !== med.id && m.isActive)
    ...
    return (
      <div className="p-6 text-center space-y-4">
        ...
        <Button asChild className="bg-blue-600 hover:bg-blue-700">
          <Link to="/medicines">Back to Store</Link>
        </Button>
      </div>
    );
  }
  ...
  const manufacturer =
    data.manufacturers.find((m) => m.id === med.manufacturerId)?.name || "Premium Labs";
    ...
    <div className="bg-white min-h-screen p-4 md:p-8 rounded-2xl shadow-sm border border-border/40 max-w-6xl mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-4 mb-8">
        <Button
          ...
          variant="ghost"
          size="sm"
          className="gap-1 pl-2 -ml-2 text-slate-500 hover:text-slate-900"
        >
          <Link to="/medicines">
        ...
        <div className="flex flex-wrap items-center text-xs md:text-sm text-muted-foreground">
          ...
          <Link to="/medicines" className="hover:text-blue-600 transition-colors">
            {category}
          </Link>
          <ChevronRight className="w-3 h-3 md:w-4 md:h-4 mx-1 md:mx-2" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mb-12">
        ...
        <div className="flex flex-col items-center">
          <div className="w-full relative aspect-square rounded-2xl bg-white border border-slate-100 p-8 flex items-center justify-center mb-6 shadow-sm overflow-hidden group">
            ...
            <img
              src={getImageForMedicine(med.id, med.dosageForm)}
              alt={med.name}
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-10">
            ...
            <div className="flex items-center border border-slate-200 rounded-xl p-1 bg-white shadow-sm self-start">
              <button
                ...
                className="p-2 md:p-3 text-slate-400 hover:text-blue-600 transition-colors"
              >
          ...
          <div className="mb-8">
            ...
            <p className="text-slate-500 text-sm leading-relaxed max-w-lg">
              {med.usageInstructions ||
                med.dosageInfo ||
                "The medicine is developed using carefully monitored chemical formulations. Inactivated components and specific excipients ensure therapeutic reliability. Efficacy levels have been clinically established without significant risks for indicated pathologies."}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8 bg-slate-50/50 p-6 rounded-xl border border-slate-100 shadow-sm">
            ...
            <div className="flex gap-3 items-start">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm font-semibold text-slate-900 mt-0.5 truncate">
                  {med.storageRequirements || "Store at room temp"}
                </p>
              </div>
            </div>
            <div className="flex gap-3 items-start">
          ...
          >
            <Heart
              className={`w-5 h-5 group-hover:scale-110 transition-transform ${isWishlisted ? "text-red-500" : ""}`}
              fill={isWishlisted ? "currentColor" : "none"}
            />
        ...
        <div className="mt-12 pt-8 border-t border-border/40">
          ...
          <div className="flex overflow-x-auto gap-4 md:gap-6 pb-6 -mx-4 px-4 md:mx-0 md:px-0 scrollbar-hide snap-x">
            {genericAlternatives.map((alt) => {
              const altPrice =
                data.batches.find((b) => b.medicineId === alt.id)?.sellingPrice || 110;
              return (
                <div
                  key={alt.id}
                  className="min-w-[200px] max-w-[200px] md:min-w-[240px] md:max-w-[240px] border border-slate-200 rounded-2xl p-4 bg-white flex flex-col snap-start shrink-0 group hover:shadow-lg transition-all relative"
                >
                  <button
                  ...
                  >
                    <Heart
                      className="w-4 h-4"
                      fill={globalWishlist.includes(alt.id) ? "currentColor" : "none"}
                    />
                  </button>
                  <Link to={`/medicines/${alt.id}`} className="flex flex-col h-full">
                    ...
                    <h3 className="font-bold text-slate-900 text-sm md:text-base truncate mb-1">
                      {alt.name}
                    </h3>
                    <p className="text-xs text-slate-500 truncate mb-3">
                      {alt.genericName || "Generic Equivalent"}
                    </p>
