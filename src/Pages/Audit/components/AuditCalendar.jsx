...
export function AuditCalendar({ audits, onSelect, onStart, onView, onNewAudit }) {
  ...
  const toggleFilter = useCallback((id, checked) => {
...
// ---------------------------------------------------------------------------
function FilterDropdown({ activeFilters, counts, onToggle, onClear }) {
  return (
  ...
  return (
    <>
      ...
      <div className="space-y-2">
        {Object.keys(AUDIT_TYPE_LABEL).map((t) => (
...
function AuditDayRow({ audit, now, onSelect, onStart, onView, mobile = false }) {
  const isOverdue =
