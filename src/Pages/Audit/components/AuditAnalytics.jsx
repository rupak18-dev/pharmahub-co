...
export function AuditAnalytics({ audits, counts, variances, medicines, categories, currency }) {
  ...
  const money = (v) => [`${formatCurrency(Number(v), currency)}`, "Variance value"];
  const moneyBar = (v) => [`${formatCurrency(Number(v), currency)}`, "Stock value"];
  return (
