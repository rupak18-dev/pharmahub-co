...
import { AuditOverview } from "@/Pages/Audit/components/AuditOverview";
import { AuditCalendar } from "@/Pages/Audit/components/AuditCalendar";
import { AuditManagement } from "@/Pages/Audit/components/AuditManagement";
import { LiveAudit } from "@/Pages/Audit/components/LiveAudit";
import { LiveAuditHome } from "@/Pages/Audit/components/LiveAuditHome";
import { VarianceReview } from "@/Pages/Audit/components/VarianceReview";
import { Approvals } from "@/Pages/Audit/components/Approvals";
...
const VIEW_ORDER = ["overview", "calendar", "audits", "live", "variance", "approvals", "activity"];
...
export default function AuditPage() {
  ...
  const [liveAuditId, setLiveAuditId] = useState(null);
  const requireUser = () => {
    if (!user) {
  ...
  const setAuditStatus = (id, patch, event, logAction, opts) => {
    db.set((d) => {
  ...
  const upsertCount = (d, auditId, batchId, patch) => {
  ...
  const submitCount = (auditId, batchId, physicalQty, extra) => {
    const u = requireUser();
    if (!u) return;
    db.set((d) => {
  ...
  const skipBatch = (auditId, batchId) => {
    const u = requireUser();
    if (!u) return;
    db.set((d) => {
  ...
  const flagBatch = (auditId, batchId, extra) => {
    const u = requireUser();
    if (!u) return;
    db.set((d) => {
  ...
  const setVarianceReason = (id, reason) => {
    db.set((d) => {
      const v = d.variances.find((x) => x.id === id);
      if (v) v.reason = reason;
    });
  };
  const setManagerComment = (id, comment) => {
    db.set((d) => {
      const v = d.variances.find((x) => x.id === id);
      if (v) v.managerComment = comment;
    });
  };
  const requestRecount = (varianceId) => {
    const u = requireUser();
    if (!u) return;
    db.set((d) => {
  ...
  const confirmRecount = (varianceId, newQty) => {
    const u = requireUser();
    if (!u) return;
    db.set((d) => {
      const v = d.variances.find((x) => x.id === varianceId);
  ...
  const applyVarianceAction = (varianceId, action, targetBranch) => {
    const u = requireUser();
    if (!u) return;
    db.set((d) => {
      const v = d.variances.find((x) => x.id === varianceId);
      ...
      const adjAction = action === "approve" ? "adjust" : action;
