...
export function purchaseGuard(expiryIso, guardDays = PURCHASE_GUARD_DAYS, now = Date.now()) {
...
export const DEFAULT_WINDOW = { kind: "preset", preset: "30" };
...
export function effectiveDays(window) {
...
export function windowSpanDays(window, now = Date.now()) {
...
export function inWindow(batch, window, now = Date.now()) {
...
export function batchValue(batch) {
  return batch.currentStock * batch.purchasePrice;
}
export function expiryBucket(days) {
...
export function suggestedDiscountPct(days) {
  if (days <= 3) return 30;
  if (days <= 7) return 25;
...
export function isReturnable(row) {
  return row.days >= -RETURN_WINDOW_DAYS && row.days <= RETURN_WINDOW_DAYS;
}
export function computeMetrics(batches, window, now = Date.now()) {
...
export function computeTrend(batches, now = Date.now(), horizonDays = 60) {
  const daily = new Map();
...
export function computeByGroup(batches, window, getGroup, now = Date.now()) {
  const map = new Map();
  for (const b of batches) {
 ...
 */
export function computeLossByGroup(batches, getGroup, now = Date.now(), horizonDays = 30) {
  const map = new Map();
  for (const b of batches) {
    if (b.status === "disposed" || b.currentStock <= 0) continue;
    const d = daysUntil(b.expiryDate, now);
 ...
 * month before `now`.
 */
export function computeMonthlyExpiry(batches, now = Date.now(), months = 6) {
...
export function computePeriodData(batches, now = Date.now()) {
  const buckets = [
...
export function computeReturnableBySupplier(batches, supplierName, now = Date.now()) {
  const map = new Map();
  for (const b of batches) {
    if (b.status === "disposed" || b.currentStock <= 0) continue;
    const d = daysUntil(b.expiryDate, now);
...
export function getAlternatives(batch, batches, medicines, now = Date.now(), opts) {
...
export function buildNotifications(batches, medicines, supplierName, now = Date.now()) {
  const medById = new Map(medicines.map((m) => [m.id, m]));
  const items = [];
  const push = (n, batch) => {
