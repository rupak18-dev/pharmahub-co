...
import type { ModuleKey, RoleName } from "@/lib/types";
...
const ROLE_META: Record<
  RoleName,
  { icon: React.ElementType; color: string; bg: string; description: string; modules: string[] }
> = {
  Owner: {
    icon: Crown,
...
function InviteWizard({ open, onClose }: { open: boolean; onClose: () => void }) {
  const profiles = useDb((d) => d.profiles);
...
function PolicyModal({
  ...
  const toggle = (mod: ModuleKey, action: string) => {
    if (!canEdit) return;
    db.set((d) => {
  ...
  const PERM_LABELS: Record<string, { label: string; desc: string; icon: React.ElementType }> = {
                  ...
                  {filteredModules.map((mod) => {
                    ...
                    return (
                      <button
                        key={mod.key}
                        type="button"
                        onClick={() => setSelectedModule(mod.key as ModuleKey)}
