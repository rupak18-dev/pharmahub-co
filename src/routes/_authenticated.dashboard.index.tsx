...
function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const stockValue = data.batches.reduce(
      (sum, b) => sum + (stockMap.get(b.id) ?? 0) * (b.purchasePrice || 0),
      0,
    );
    const now = Date.now();
    ...
    data.batches.forEach((b) => {
      perMedicine.set(
        b.medicineId,
        (perMedicine.get(b.medicineId) ?? 0) + (stockMap.get(b.id) ?? 0),
      );
    });
    ...
    const near = data.batches.filter((b) => {
      ...
      return (stockMap.get(b.id) ?? 0) > 0 && t > now && t - now <= nearMs;
    }).length;

    const expired = data.batches.filter(
      (b) => new Date(b.expiryDate).getTime() <= now && (stockMap.get(b.id) ?? 0) > 0,
    ).length;
