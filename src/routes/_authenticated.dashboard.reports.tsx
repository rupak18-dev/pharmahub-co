...
export const Route = createFileRoute("/_authenticated/dashboard/reports")({
  head: () => ({ meta: [{ title: "Reports Center · PharmacyOS Enterprise" }] }),
...
type ReportStatus = "Ready" | "Scheduled" | "Uploaded" | "Archived";

interface ReportDefinition {
  id: string;
  title: string;
  category: Exclude<ReportCategory, "All" | "Favorites">;
  description: string;
  owner: string;
...
interface ActivityItem {
  id: string;
  reportTitle: string;
  action: "Exported" | "Previewed" | "Generated" | "Uploaded";
  format?: ExportFormat;
  user: string;
  timestamp: Date;
}
...
type SortField = "title" | "category" | "owner" | "lastGenerated" | "status";
...
const CATEGORY_LIST: { id: ReportCategory; label: string; icon: React.ElementType }[] = [
...
function ReportsPage() {
  ...
  // Primary State
  const [reports, setReports] = useState<ReportDefinition[]>(INITIAL_REPORTS);
  ...
  const [newReportCategory, setNewReportCategory] =
    useState<Exclude<ReportCategory, "All" | "Favorites">>("Sales");
  ...
  const [uploadCategory, setUploadCategory] =
    useState<Exclude<ReportCategory, "All" | "Favorites">>("Compliance");
  ...
  const categoryCounts = useMemo(() => {
    const map: Record<ReportCategory, number> = {
      All: reports.length,
    ...
    switch (reportId) {
      ...
      case "stock-valuation": {
        const catVal = new Map<
          string,
          { count: number; units: number; costVal: number; mktVal: number }
        >();
      ...
      case "gst-summary": {
        ...
        const gstByRate = new Map<number, { count: number; taxable: number; tax: number }>();
        sales.forEach((s) =>
          s.items.forEach((it) => {
  ...
  // Export Action
  const handleExport = (report: ReportDefinition, fmt: ExportFormat) => {
    if (dateError) {
  ...
  // Open Preview Modal
  const handleOpenPreview = (report: ReportDefinition) => {
    if (dateError) {
  ...
  const handleCreateNewReportSubmit = (e: React.FormEvent) => {
    ...
    const createdReport: ReportDefinition = {
      id: newId,
  ...
  const handleUploadReportSubmit = (e: React.FormEvent) => {
    ...
    const uploadedReport: ReportDefinition = {
      id: newId,
      title: docTitle,
  ...
  const handleDeleteReport = (report: ReportDefinition) => {
                      ...
                      >
                        <Star
                          className={`h-3.5 w-3.5 ${
                            report.isPinned ? "text-amber-500 fill-amber-500" : ""
                          }`}
                        />
                      </button>
                    </div>
          ...
          <form onSubmit={handleCreateNewReportSubmit} className="space-y-3.5 py-1">
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setNewReportCategory(v as Exclude<ReportCategory, "All" | "Favorites">)
                }
              >
          ...
          <form onSubmit={handleUploadReportSubmit} className="space-y-3.5 py-1">
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setUploadCategory(v as Exclude<ReportCategory, "All" | "Favorites">)
                }
              >
