...
import { ensureDevelopmentUser } from "../src/services/devUser.service.js";
...
async function run() {
  ...
  // User accounts:
  //   development         ensures the single demo login account
  //                       (demo@pharmahub.local / PharmaHub@123, Owner)
  //   --owner-email=…     seeds one REAL Owner account (password: --owner-password
  //                       or OWNER_PASSWORD env, default "password123")
  // No other demo users, staff or invitations are ever seeded.
  const ownerEmailFlag = process.argv.find((a) => a.startsWith("--owner-email="));
  const ownerEmail = ownerEmailFlag ? ownerEmailFlag.split("=")[1] : null;
  const ownerPasswordFlag = process.argv.find((a) => a.startsWith("--owner-password="));
  const ownerPassword =
    (ownerPasswordFlag ? ownerPasswordFlag.split("=")[1] : null) ??
    process.env.OWNER_PASSWORD ??
    "password123";

  const users = [];
  if (ownerEmail) {
    users.push({ name: "Store Owner", email: ownerEmail, role: "Owner", orgName: "PharmaHub" });
  }
  let userCount = 0;
  ...
  const devUser = await ensureDevelopmentUser();
  if (devUser.created) userCount += 1;
  ...
  console.log("");
  if (ownerEmail) {
    console.log(`  Owner account created: ${ownerEmail} (password set via --owner-password / OWNER_PASSWORD)`);
  }
  if (env.nodeEnv !== "production") {
    console.log("  Development demo login (created only if missing):");
    console.log(`    email   : demo@pharmahub.local`);
    console.log(`    password: PharmaHub@123`);
    console.log(`    role    : Owner`);
  }
  if (!ownerEmail) {
    console.log("");
    console.log("  To add a real Owner account:");
    console.log("    npm run seed -- --owner-email=owner@example.com  (one real Owner account)");
  }
