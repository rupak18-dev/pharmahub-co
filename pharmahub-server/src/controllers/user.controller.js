...
function assertSameOrganization(invitation, caller) {
  ...
  if (invitation.orgName && invitation.orgName === caller.orgName) return;
...
function assertSameOrgOrAllow(target, caller) {
  ...
  if (target.orgName !== caller.orgName) {
...
export const listUsers = asyncHandler(async (req, res) => {
  ...
  if (req.user.orgName) filter.orgName = req.user.orgName;
...
export const updateUser = asyncHandler(async (req, res) => {
  ...
  if (roleChanged) updatePayload.roleId = (await resolveRoleId(role)) ?? null;
  if (status !== undefined) {
  ...
  if (shouldNotify) {
    ...
    const { subject, html, text } = buildRoleChangeEmail({
      ...
      previousPermissions: kind === "both" ? previousPermissions : undefined,
...
// audit trail instead. Only non-secret details (recipient, subject) are ever
// recorded.
...
export const deleteUser = asyncHandler(async (req, res) => {
  ...
  return ok(res, toPublicUser(user.toObject()), "User removed");
});

export const inviteUser = asyncHandler(async (req, res) => {
  const { name, email, role, message, phone, permissions, featureAccess, accessIds, department } = req.body;
  logger.info(
...
export const listInvitations = asyncHandler(async (req, res) => {
  logger.info(`[users.invitations] GET /users/invitations — by=${req.user.email} org=${req.user.orgName ?? "(none)"}`);
  // Accepted/used invitations are lifecycle records of users who already
  // joined — active staff membership is driven exclusively by the users
  // collection. Excluding them here guarantees a user who was removed from the
  // organization can never be resurrected by a stale invitation row.
  const filter = {
    status: { $nin: ["accepted", "used"] },
    ...(req.user.orgName
      ? { $or: [{ orgName: req.user.orgName }, { invitedBy: req.user._id }] }
      : { invitedBy: req.user._id }),
  };
