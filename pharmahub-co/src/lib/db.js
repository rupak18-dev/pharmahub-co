...
function load() {
  ...
  try {
    ...
    if (raw) {
      ...
      const seeded = seed();
      // Strip legacy dummy/demo profiles and placeholder names so the local
      // database never shows fake staff. All @pharmahub.demo accounts are
      // removed (including the old owner@pharmahub.demo). Real staff come
      // from the backend and are mirrored via upsertBackendUser; the
      // development demo login account is a real backend user and is
      // mirrored normally.
      const cleanProfiles = (loaded.profiles ?? []).filter((p) => {
        const email = String(p.email || "").toLowerCase();
        const isDemo = email.endsWith("@pharmahub.demo");
        return !isDemo && !["Alex Morgan", "Priya Shah", "Sam Chen", "Diego Ruiz"].includes(p.name);
      });
