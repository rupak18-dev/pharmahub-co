...
export default function ReportBuilder({
  ...
  // Filters can target dimension fields OR numeric measures (e.g. "Grand Total
  // greater than 500"). The backend validates and materializes measure values
  // only when a filter actually uses them.
  const filterFields = useMemo(() => [...module.fields, ...module.measures], [module]);
  ...
  const customRangeValue =
    customFrom || customTo
      ? [
          customFrom ? dayjs(customFrom, "YYYY-MM-DD") : null,
          customTo ? dayjs(customTo, "YYYY-MM-DD") : null,
        ]
      : null;
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <Input
                ...
                placeholder={title}
                className="h-8 border-0 bg-transparent px-0 text-base font-semibold text-foreground placeholder:text-muted-foreground/50 focus-visible:ring-0 focus-visible:ring-offset-0 shadow-none min-w-0 flex-1"
              />
              {module.dataSource && (
                <Badge
                  variant="secondary"
                  className="h-5 shrink-0 text-[10px] font-medium gap-1"
                  title={`Report data source: ${module.dataSource}`}
                >
      ...
      <div className="flex flex-col lg:flex-row gap-4 flex-1 min-h-0">
        ...
        <div className="w-full lg:w-80 xl:w-96 shrink-0 flex flex-col gap-3">
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <FilterBuilder filters={filters} availableFields={filterFields} onChange={setFilters} />
          </div>
        </div>
