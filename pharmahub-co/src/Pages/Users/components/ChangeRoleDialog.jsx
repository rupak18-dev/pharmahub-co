...
import { getRoleMeta, defaultModulesForRole } from "./staffRoles";
...
export function ChangeRoleDialog({ open, onOpenChange, profile, onSave }) {
  ...
  // Tracks the role currently loaded from the profile so the rebase effect can
  // tell "dialog opened with this role" apart from "Owner picked a new role".
  const lastProfileRole = useRef(null);
  ...
  useEffect(() => {
    if (open && profile) {
      ...
      // Preload the access chips from the profile's explicit whitelist when one
      // exists, otherwise from the role's default modules so a plain role-only
      // save is a no-op on access.
      setAccessTouched(false);
      setAccess(
        profile.accessIds?.length
          ? profile.accessIds
          : defaultModulesForRole(profile.role ?? "Pharmacist"),
      );
      lastProfileRole.current = profile.role ?? "Pharmacist";

      const isAdmin = profile.role === "Owner" || profile.role === "Admin";
      const savedFeatures = profile.featureAccess ?? {};
      setFeatures({
        processSales: savedFeatures.processSales !== false,
        stockAudit: savedFeatures.stockAudit !== false,
        purchasing: savedFeatures.purchasing !== false,
        dataExport: savedFeatures.dataExport !== false,
        notifications: savedFeatures.notifications !== false,
        userAdmin: savedFeatures.userAdmin === true || isAdmin,
      });
    }
  }, [open, profile]);

  // Switching the selected role rebases the access chips to that role's default
  // modules, but never clobbers a whitelist the Owner has manually adjusted.
  useEffect(() => {
    if (!open || accessTouched) return;
    if (role === lastProfileRole.current) return;
    setAccess(defaultModulesForRole(role));
    lastProfileRole.current = role;
  }, [role, open, accessTouched]);

  const meta = getRoleMeta(role);
  const addAccess = (ids) => {
    setAccessTouched(true);
    setAccess((prev) => [...new Set([...prev, ...ids])]);
  };
  const removeAccess = (id) => {
    setAccessTouched(true);
    setAccess((prev) => prev.filter((x) => x !== id));
  };
