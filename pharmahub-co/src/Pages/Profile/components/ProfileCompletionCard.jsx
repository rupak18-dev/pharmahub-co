...
export function ProfileCompletionCard({ profile, onSelectField }) {
  ...
  // The backend denormalizes all profile/organization fields onto each user
  // record (mirrored into the local profile), so completion is computed from
  // the signed-in user's own data — no separate Owner lookup.
  const computed = calculateProfileCompletion(liveProfile, liveProfile);
  // Prefer the server-computed score (same 13 fields, persisted on the user);
  // fall back to the client-side calculation for local/offline profiles.
  const percentage =
    typeof liveProfile?.profileCompletion?.percentage === "number"
      ? liveProfile.profileCompletion.percentage
      : computed.percentage;
  const { missingFields, missingSpecs } = computed;
  ...
  const handlePillClick = (section) => {
    if (onSelectField) {
      ...
      window.location.href = `/profile/edit#${section}`;
    }
  };
