...
export const handle = { title: "AI Insights · PharmaHub" };
export default function AiPage() {
  ...
  const insights = useMemo(() => {
    ...
    const w1 = new Map(); // last 7d
    const w2 = new Map(); // 8-14d
    data.inventoryLedger.forEach((m) => {
      if (
        m.movementType === "Purchase Inward" ||
        m.movementType === "Customer Return" ||
        (m.movementType === "Adjustment" && m.quantityChange > 0)
      )
        return;
    ...
    const gainers = [...trend]
      .filter((t) => t.recent >= 5 && t.delta > 10)
      .sort((a, b) => b.delta - a.delta)
      .slice(0, 5);
    const losers = [...trend]
      .filter((t) => t.prior >= 5 && t.delta < -10)
    ...
    <div className="space-y-6">
      ...
      {empty ? (
        <EmptyState
          ...
          description="Record a few sales and purchases so insights can compute."
          icon={Sparkles}
        />
      ) : (
          ...
          <Card icon={AlertTriangle} title="Suggested reorders">
            {insights.reorder.length ? (
              ...
              <p className="text-sm text-muted-foreground">
                No reorders projected in next 14 days.
              </p>
            )}
          </Card>
