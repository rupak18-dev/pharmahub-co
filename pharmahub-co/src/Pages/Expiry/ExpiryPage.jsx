...
import { ExpiryInventory } from "@/Pages/Expiry/components/ExpiryInventory";
import { ExpiryReports } from "@/Pages/Expiry/components/ExpiryReports";
import { ExpiryLedgers } from "@/Pages/Expiry/components/ExpiryLedgers";
import { RecommendationPanel } from "@/Pages/Expiry/components/RecommendationPanel";
import { BatchDrawer } from "@/Pages/Expiry/components/BatchDrawer";
import { MedicineDrawer } from "@/Pages/Expiry/components/MedicineDrawer";
import {
  DiscountDialog,
  ReturnDialog,
  TransferDialog,
} from "@/Pages/Expiry/components/ExpiryDialogs";
import {
  BRANCHES,
  DEFAULT_WINDOW,
  buildNotifications,
...
export default function ExpiryPage() {
  ...
  const [recBatchId, setRecBatchId] = useState(null);
  ...
  const medicineCategory = useCallback(
    (id) => catById.get(medById.get(id)?.categoryId ?? "") ?? "—",
    [medById, catById],
  );
  const medicineManufacturer = useCallback(
    (id) => mfrById.get(medById.get(id)?.manufacturerId ?? "") ?? "—",
    [medById, mfrById],
  );
  ...
  const doReturn = (row, qty, netValue, creditNoteNo) => {
    const u = requireUser();
    if (!u) return;
    applyStockMovement({
  ...
  const doTransfer = (row, branch, qty) => {
    const u = requireUser();
    if (!u) return;
  ...
  const markCreditReceived = (id, creditNoteNo) => {
    db.set((d) => {
  ...
  const openRecommendations = (batchId) => {
  ...
  const exportReport = (kind, format) => {
