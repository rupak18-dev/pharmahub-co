...
export function ConnectIntegrationDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isConfigure) {
        toast.success(`${item.name} configuration saved`);
      } else if (record?.status === "connected") {
        toast.success(`${item.name} connected successfully`);
      } else {
        toast.success(
          `${item.name} configuration saved. Connecting ${item.name} requires provider credentials that are not available in this build.`,
        );
      }
      onOpenChange(false);
      onDone?.(record);
      ...
      toast.error(
        err?.message ??
          `Unable to ${isConfigure ? "update" : "connect"} ${item.name}. Please try again.`,
      );
      setSubmitting(false);
    }
  };
