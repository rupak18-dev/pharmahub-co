  ...
  return (
    <Card className="overflow-hidden border-border bg-card shadow-sm transition-all duration-200 hover:shadow-md">
      ...
      <CardHeader className="border-b border-border bg-muted/20 px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{user?.orgName || "Not provided"}</p>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-5 px-5 py-4">
        {/* Avatar + name row */}
        <div className="flex items-center gap-3">
          <Avatar className="h-10 w-10 shrink-0 rounded-full border border-border bg-primary/10">
            {user?.avatarUrl && (
              <AvatarImage
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-foreground">
              {user?.name || (
                <span className="italic text-muted-foreground font-normal">Not provided</span>
              )}
            </p>
