...
export function InviteStaffDrawer({ open: controlledOpen, onOpenChange: controlledOnOpenChange }) {
  ...
  // Note: profiles is kept for other potential read-only uses; the email
  // duplicate check was removed — the backend validates uniqueness and returns
  // HTTP 409 which the catch block surfaces as a toast. A local check risks
  // false positives when the local DB is stale or the user is in a removed/
  // pending-only state that the server permits re-inviting.
  ...
  const validateDetails = () => {
    ...
    // NOTE: removed local-profile duplicate check here. The backend validates
    // uniqueness and returns HTTP 409 Conflict which the catch block surfaces.
  ...
  // Derive the per-user permission OVERRIDES from the Step 2 module toggles.
  // Only modules that deviate from the role's default become explicit overrides
  // (the backend merges them over the role defaults). Revoking a module sets
  // every action to false; granting one the role denies enables view.
  const buildPermissionOverrides = () => {
    const roleDefaults = DEFAULT_PERMISSIONS[role] || {};
    const overrides = {};
    ALL_MODULES.forEach((m) => {
      const enabled = moduleAccess[m.key] ?? true;
      const roleAllowsView = roleDefaults[m.key]?.view ?? false;
      if (!enabled) {
        overrides[m.key] = {
          view: false,
          create: false,
      ...
      } else if (!roleAllowsView) {
        overrides[m.key] = { view: true };
      }
    });
    return overrides;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const data = await invitationService.invite({
        name: name.trim(),
        email: email.trim(),
        phone: phone.trim() ? normalizePhone(phone) : undefined,
        role,
        department,
        accessIds: Object.entries(moduleAccess)
          .filter(([_, enabled]) => enabled)
          .map(([key]) => key),
        permissions: buildPermissionOverrides(),
        featureAccess: {
          dashboard: allowDashboardAccess,
          mobile: allowDashboardAccess,
          ...features,
        },
      });
      ...
      if (data?.emailSkipped) {
        toast.info(
          "Invitation created — email delivery was skipped because SMTP is not configured.",
          { duration: 8000 },
        );
      } else {
      ...
      if (data?.link) {
        toast.info("Share this link manually if the email was not received:", {
          duration: 15000,
          action: {
            label: "Copy Link",
            onClick: () => {
      ...
      // A TypeError means the backend is unreachable; anything else is a real
      // API error. Never fake success or mirror the invite locally — the
      // backend is the single source of truth.
      const message = error instanceof Error ? error.message : "Failed to send the invitation.";
      toast.error(message);
    } finally {
      submittingRef.current = false;
      setSubmitting(false);
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className={`pl-9 text-xs rounded-xl ${errors.phone ? "border-destructive focus-visible:ring-destructive/30" : ""}`}
                    />
                  </div>
